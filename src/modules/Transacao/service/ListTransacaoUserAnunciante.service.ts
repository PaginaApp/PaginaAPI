...
@injectable()
class ListTransacaoUserAnuncianteService {
  constructor(
  ...
  async execute(
    ...
    if (!data.filter?.trs_usu_Anunciante_id) {
      throw new EntityNotFoundError('Usuário não encontrado');
    }

    const user = await this.userRepository.findBy({
      usu_Id: data.filter.trs_usu_Anunciante_id,
    });
...
export { ListTransacaoUserAnuncianteService };
